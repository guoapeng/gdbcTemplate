...
func (mapper *beanPropertyRowMapper) RowMapper(row *sql.Row) interface{} {
	...
	for i := 0; i < ss.NumField(); i++ {
		...
		zap.S().Debugf("tag:%s, tag value:%s, filed name:%s", ss.Type().Field(i).Tag.Get("sql"), val, name)
		...
		case reflect.Int, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			if err != nil {
				zap.S().Infof("can't not atoi:%v", val)
				continue
			}
		...
		case reflect.Float32, reflect.Float64:
			...
			if err != nil {
				zap.S().Infof("can't not ParseFloat:%v", val)
				continue
			}
			ss.FieldByName(name).SetFloat(f)
		default:
			zap.S().Infof("unknown type:%+v", ss.Field(i).Kind())
		}
	}
	return out
}
...
func (mapper *beanPropertyRowMapper) RowsMapper(rows *sql.Rows) interface{} {
	...
	for i := 0; i < ss.NumField(); i++ {
		...
		zap.S().Error(err)
		name := ss.Type().Field(i).Name
		zap.S().Debugf("tag:%s, tag value:%s, field name:%s", ss.Type().Field(i).Tag.Get("sql"), val, name)
		switch ss.Field(i).Kind() {
		case reflect.String:
			ss.FieldByName(name).SetString(val)
		case reflect.Int, reflect.Int16, reflect.Int32, reflect.Int64:
			i, err := strconv.Atoi(val)
			if err != nil {
				zap.S().Errorf("can't not atoi:%v", val)
				continue
			}
			ss.FieldByName(name).SetInt(int64(i))
		case reflect.Uint16, reflect.Uint32, reflect.Uint64:
		...
		case reflect.Float32, reflect.Float64:
			f, err := strconv.ParseFloat(val, 64)
			if err != nil {
				zap.S().Errorf("can't not ParseFloat:%v", val)
				continue
			}
			ss.FieldByName(name).SetFloat(f)
		default:
			zap.S().Errorf("unknown type:%+v", ss.Field(i).Kind())
		}
	}
	return out
}
